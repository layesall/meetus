      ...
      {!loading && !error && events.length > 0 && (
        <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {events.map((ev) => (
            ...
            >
              <span
                className="mb-4 inline-block h-1.5 w-12 rounded-full"
              ...
              <Link
                href={`/book/${ev.slug}`}
                className={`mt-6 inline-flex items-center justify-center rounded-lg bg-neutral-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-600`}
              >
                Réserver
              </Link>
            </li>
...
function StateMessage({
  ...
  return (
    <div className={`rounded-2xl border px-6 py-10 text-center text-sm ${styles}`}>
      {text}
    </div>
  );
}